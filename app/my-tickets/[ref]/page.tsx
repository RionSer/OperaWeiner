...
import { ArrowLeft, Calendar, Clock, Download, MapPin, Music, QrCode, Ticket } from "lucide-react"
...
export default async function MyTicketDetailPage({
  ...
  const email =
    ...
    (bypassAuth ? ((emailParam || "").trim().toLowerCase() || fallbackEmail) : "")
  if (!email) {
    redirect("/find-tickets")
  }
  ...
  const fullName = booking.email?.split("@")[0] || "Guest"
            ...
            <div className="bg-primary px-4 py-4 sm:px-6">
              <div className="flex min-w-0 items-center justify-between gap-3">
                <div className="flex min-w-0 items-center gap-2 text-primary-foreground">
                  ...
                  <span className="truncate font-serif text-base font-bold sm:text-lg">Wiener Oper</span>
                </div>
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                <div className="flex items-start gap-3 rounded-sm bg-secondary p-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 font-medium text-card-foreground">{booking.concert_venue}</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 rounded-sm bg-secondary p-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 font-medium text-card-foreground">
                      {format(parseISO(booking.concert_date), "EEEE, MMMM d, yyyy")}
                    </p>
                  </div>
                </div>
                <div className="flex items-start gap-3 rounded-sm bg-secondary p-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 font-medium text-card-foreground">{booking.concert_time}</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 rounded-sm bg-secondary p-4">
                  <Ticket className="mt-0.5 h-5 w-5 shrink-0 text-accent" />
                  <div>
                    <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Tickets</p>
                    <p className="mt-1 font-medium text-card-foreground">
              ...
              <div className="flex min-w-0 flex-col items-center gap-4 sm:flex-row sm:justify-between sm:items-start">
                <div className="flex w-full min-w-0 items-center gap-4 sm:w-auto">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">Booked on {bookingDateFormatted}</p>
                  </div>
                </div>
